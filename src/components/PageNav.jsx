...
const primaryLinks = [
  { key: 'explore', to: '/explore', label: 'Explore', icon: 'explore', description: 'Global public discovery' },
  { key: 'library', to: '/library', label: 'My Library', icon: 'dashboard', description: 'Your private picks and ratings' },
  { key: 'cliques', to: '/cliques', label: 'Cliques', icon: 'users', description: 'Shared voting spaces' },
]

const pageMeta = {
  explore: { label: 'Explore', icon: 'explore' },
  library: { label: 'My Library', icon: 'dashboard' },
  cliques: { label: 'Cliques', icon: 'users' },
...
const librarySectionKeys = new Set(['library', 'movies', 'series', 'games', 'videos', 'music'])

function normalizeActiveKey(active) {
  if (active === 'home') return 'library'
  if (active === 'groups') return 'cliques'
  if (active === 'leaderboard') return 'explore'
  return active || 'library'
}

function isPrimaryActive(linkKey, activeKey) {
  if (linkKey === activeKey) return true
  if (linkKey === 'library') return librarySectionKeys.has(activeKey)
  return false
}
...
export default function PageNav({ active = 'library' }) {
  ...
  const activeMeta = pageMeta[activeKey] || pageMeta.library
  ...
  const groupLabel = activeGroup?.name || 'My Library'
  const showContextSwitcher = activeKey !== 'explore'
  ...
  async function handleTogglePublic(group) {
    if (!session?.user || !hasSupabase) {
      flash('Public discovery is available for signed-in Supabase cliques.')
      return
    }
    ...
    <>
      <header className="relative z-40 mb-5 rounded-[2rem] border border-white/10 bg-neutral-950/95 px-3 py-3 shadow-2xl shadow-black/30 backdrop-blur sm:px-4">
        <div className="grid gap-3 md:grid-cols-[auto_1fr_auto] md:items-center">
          <Link to="/explore" aria-label="CliqueBase global Explore" className="min-w-0 rounded-[1.4rem] px-2 py-1 transition hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-white/30" onClick={closeSwitchers}>
            <LogoMark />
          </Link>
          ...
          <div className="relative flex min-w-0 justify-center">
            <div className="flex w-full min-w-0 max-w-xl flex-col items-stretch justify-center gap-2 sm:flex-row sm:items-center">
              ...
              {showContextSwitcher ? (
                ...
                <div className="flex min-w-0 items-center justify-center rounded-full border border-white/10 bg-white/[0.035] px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.18em] text-neutral-500">
              ...
              <div className="absolute left-1/2 top-full mt-3 w-[min(92vw,24rem)] -translate-x-1/2 rounded-[2rem] border border-white/10 bg-neutral-950 p-3 shadow-2xl shadow-black/50">
                <div className="grid gap-2">
                  {primaryLinks.map((link) => (
                    <Link key={link.key} to={link.to} onClick={closeSwitchers} className={`flex items-center gap-3 rounded-2xl px-4 py-3 transition ${isPrimaryActive(link.key, activeKey) ? 'bg-white font-bold text-neutral-950' : 'bg-white/[0.04] text-neutral-200 hover:bg-white/10 hover:text-white'}`}>
            ...
            {showContextSwitcher && groupsOpen ? (
              <div className="absolute left-1/2 top-full mt-3 w-[min(92vw,32rem)] -translate-x-1/2 rounded-[2rem] border border-white/10 bg-neutral-950 p-3 shadow-2xl shadow-black/50">
                <div className="flex items-center justify-between gap-3 px-2">
                  <div>
                    <div className="text-xs uppercase tracking-[0.3em] text-neutral-500">Viewing</div>
                ...
                <div className="mt-3 space-y-2">
                  <button type="button" onClick={usePersonalLibrary} className={`flex w-full items-center justify-between gap-3 rounded-2xl px-4 py-3 text-left transition ${!activeGroup ? 'bg-white text-neutral-950' : 'bg-white/[0.04] text-white hover:bg-white/10'}`}>
                    <span><strong>My Library</strong><span className="block text-xs opacity-60">Private picks before sharing with a clique</span></span>
                  ...
                  {!groups.length ? <p className="rounded-2xl border border-dashed border-white/15 p-4 text-sm text-neutral-500">Create a clique or join an invite from Manage.</p> : null}
                </div>
              </div>
            ) : null}
          </div>
          ...
          <div className="flex shrink-0 justify-end">
            <button type="button" onClick={() => { refreshGroups(); closeSwitchers(); setEditing(true) }} aria-label="Open profile and cliques" className="flex h-11 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] px-3 text-sm font-semibold text-white transition hover:bg-white hover:text-black sm:px-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-white/10 bg-neutral-950 p-5 shadow-2xl shadow-black/40">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-xs uppercase tracking-[0.3em] text-neutral-500">Account & cliques</div>
                <h2 className="mt-1 text-2xl font-bold text-white">Your CliqueBase setup</h2>
                <p className="mt-2 text-sm text-neutral-400">Manage your account, My Library, and shared clique spaces.</p>
              </div>
            ...
            <section className="mt-5 rounded-3xl border border-white/10 bg-white/[0.03] p-4">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div className="flex min-w-0 flex-1 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 truncate text-sm text-neutral-400">{session?.user?.email || (hasSupabase ? 'Use an account to sync My Library and cliques.' : 'Local profile mode')}</p>
                  </div>
                </div>
              ...
              {profileEditOpen && (session?.user || !hasSupabase) ? (
                <div className="mt-4 rounded-2xl bg-neutral-900 p-3">
                  ...
                  <p className="mt-2 text-xs text-neutral-500">This name appears on picks, cliques, and My Library.</p>
                </div>
              ) : null}
            ...
            <section className="mt-4 rounded-3xl border border-white/10 bg-white/[0.03] p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div className="text-xs uppercase tracking-[0.3em] text-neutral-500">Viewing</div>
                  <h3 className="mt-1 text-xl font-bold text-white">{activeGroup?.name || 'My Library'}</h3>
                  <p className="mt-1 text-sm text-neutral-400">Use cliques for shared voting. My Library stays private.</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={usePersonalLibrary} className="rounded-2xl border border-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white hover:text-neutral-950">View My Library</button>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <form onSubmit={handleCreateGroup} className="rounded-3xl border border-white/10 bg-white/[0.03] p-4">
                <label className="text-sm font-semibold text-neutral-300">Create clique</label>
              ...
              <form onSubmit={handleJoinGroup} className="rounded-3xl border border-white/10 bg-white/[0.03] p-4">
                ...
                <button className="mt-3 w-full rounded-2xl border border-white/10 px-5 py-3 font-semibold text-white hover:bg-white hover:text-neutral-950">Join clique</button>
              </form>
            </div>
          </div>
        </div>
