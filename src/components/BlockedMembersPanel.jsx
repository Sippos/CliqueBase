    ...
    <article className="rounded-2xl border border-white/10 bg-white/[0.045] p-3 backdrop-blur">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-neutral-400">{incoming ? 'Wants to be friends' : 'Request sent'}</p>
        </div>
        {incoming ? (
      ...
      {open ? (
        <div className="mt-4 grid gap-4">
          ...
          <div className="grid gap-2">
            ...
            {friends.length ? friends.map((friend) => <PersonCard key={friend.id} person={friend} action={handleRemoveFriend} actionLabel="Remove" busy={busyKey === `remove:${friend.id}`} friend />) : <p className="rounded-2xl border border-dashed border-white/10 bg-black/15 p-3 text-sm text-neutral-400">No friends yet.</p>}
          </div>

          <button type="button" onClick={() => setShowSafety((value) => !value)} className="text-left text-xs font-black uppercase tracking-[0.16em] text-neutral-500 hover:text-neutral-100">{showSafety ? 'Hide blocked users' : `Blocked users (${blockedMembers.length})`}</button>
          {showSafety ? (
            <div className="grid gap-2">
              {blockedMembers.length ? blockedMembers.map((member) => (
                <article key={member.id} className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/20 p-3">
                  <div className="min-w-0"><p className="truncate text-sm font-black text-white">{member.displayName}</p>{member.blockedAt ? <p className="mt-1 text-xs text-neutral-400">Blocked {new Date(member.blockedAt).toLocaleDateString()}</p> : null}</div>
                  <button type="button" disabled={busyKey === `unblock:${member.id}`} onClick={() => handleUnblock(member)} className="rounded-xl border border-white/10 bg-white/[0.06] px-3 py-2 text-xs font-black text-neutral-100 transition hover:bg-white hover:text-neutral-950 disabled:opacity-50">{busyKey === `unblock:${member.id}` ? 'Unblocking…' : 'Unblock'}</button>
                </article>
              )) : <p className="rounded-2xl border border-dashed border-white/10 bg-black/15 p-3 text-sm text-neutral-400">No blocked members.</p>}
            </div>
          ) : null}
        </div>
      ) : null}
