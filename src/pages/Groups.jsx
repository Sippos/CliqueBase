  ...
  return (
    <div className="flex min-w-0 flex-wrap items-end gap-2">
      {list.slice(0, 4).map((member, index) => {
        ...
        const content = <><span className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-white/15 bg-white text-[10px] font-black text-neutral-950 shadow-md shadow-black/20">{memberInitial(member)}</span><span className="max-w-full truncate text-[10px] font-bold leading-none text-neutral-300">{name}</span></>
        return interactive ? (
          <Link key={`${name}-${index}`} to={memberLibraryPath(member)} title={`Open ${name}'s library`} className="flex max-w-[4.75rem] flex-col items-center gap-1 rounded-2xl border border-white/10 bg-white/[0.035] px-2 py-1.5 text-center shadow-lg shadow-black/10 transition hover:-translate-y-0.5 hover:border-white/25 hover:bg-white/[0.07]">
            {content}
          </Link>
        ) : (
          <span key={`${name}-${index}`} title={name} className="flex max-w-[4.75rem] flex-col items-center gap-1 rounded-2xl border border-white/10 bg-white/[0.035] px-2 py-1.5 text-center shadow-lg shadow-black/10">{content}</span>
        )
      })}
...
function MetricBox({ value, label, icon }) {
  return <div className="flex min-w-0 items-center gap-2 rounded-2xl border border-white/10 bg-neutral-950/65 px-3 py-2"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/[0.06] text-neutral-300"><AppIcon name={icon} size={13} /></span><span className="min-w-0"><span className="block text-lg font-black leading-none text-white">{value}</span><span className="block text-[9px] font-bold uppercase tracking-[0.16em] text-neutral-500">{label}</span></span></div>
}

function TopContentTile({ group, category, onOpenItem, onOpenList }) {
  const item = category.top
  const image = item?.backdrop || item?.poster
  if (!item) return <button type="button" onClick={() => onOpenList(group, category)} className="min-h-[7.25rem] rounded-[1.25rem] border border-dashed border-white/10 bg-neutral-950/45 p-3 text-left transition hover:border-white/25 hover:bg-neutral-900"><div className="inline-flex items-center gap-1.5 rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-neutral-300"><AppIcon name={category.icon} size={12} />{category.title}</div><p className="mt-3 text-xs text-neutral-500">No {category.title.toLowerCase()} yet.</p></button>
  return <article className="group relative min-h-[7.25rem] overflow-hidden rounded-[1.25rem] border border-white/10 bg-neutral-950/75 transition hover:border-white/25 hover:bg-neutral-900"><button type="button" onClick={() => onOpenItem({ ...item, category: category.singular, icon: category.icon })} className="block h-full min-h-[7.25rem] w-full text-left" aria-label={`Open ${item.title} details`}>{image ? <img src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-68 transition group-hover:scale-105" /> : null}<div className="absolute inset-0 bg-gradient-to-t from-black via-black/55 to-black/10" /><span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-white px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-neutral-950"><AppIcon name={category.icon} size={11} strokeWidth={2.5} />Top {category.singular}</span><span className="absolute right-3 top-3 rounded-full bg-black/55 px-2 py-1 text-[10px] font-black text-white backdrop-blur">{category.count}</span><div className="absolute inset-x-0 bottom-0 p-3"><h3 className="line-clamp-1 text-base font-black leading-tight text-white">{item.title}</h3><p className="mt-1 line-clamp-1 text-[11px] font-semibold text-neutral-400">Score {item.score || 0} · {item.picks || 0} picks{item.rating ? ` · ★ ${Number(item.rating).toFixed(1)}` : ''}</p></div></button></article>
}
...
function GroupContentOverview({ group, summary, loading, onOpenItem, onOpenList }) {
  ...
  return <div className="mt-4 border-t border-white/10 pt-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><p className="text-[10px] font-black uppercase tracking-[0.24em] text-neutral-500">Content overview</p><div className="grid grid-cols-3 gap-2 sm:min-w-[16rem]"><MetricBox value={safeSummary.items} label="Items" icon="dashboard" /><MetricBox value={safeSummary.score} label="Score" icon="explore" /><MetricBox value={safeSummary.done} label="Done" icon="info" /></div></div><div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">{safeSummary.categories.map((category) => <TopContentTile key={category.title} group={group} category={category} onOpenItem={onOpenItem} onOpenList={onOpenList} />)}</div></div>
}

function GroupCard({ group, summary, summaryLoading, onCopy, onOpen, onOpenItem, onOpenList }) {
  const memberCount = group.members?.length || 1
  return <article className="rounded-[1.65rem] border border-white/10 bg-white/[0.028] p-4 text-white shadow-xl shadow-black/10 transition hover:border-white/20 hover:bg-white/[0.04]"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.24em] text-neutral-500">{group.isPublic ? 'Public clique' : 'Private clique'}</p><h2 className="mt-1 truncate text-2xl font-black leading-tight">{group.name}</h2><div className="mt-3 flex flex-wrap items-end gap-3 text-xs text-neutral-400"><MemberAvatars members={group.members || []} /><span className="mb-1 rounded-full border border-white/10 px-2.5 py-1 font-bold">{memberCount} {memberCount === 1 ? 'member' : 'members'}</span></div></div><div className="flex shrink-0 flex-wrap gap-2"><button type="button" onClick={() => onCopy(group)} aria-label={`Copy invite for ${group.name}`} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-white transition hover:bg-white hover:text-neutral-950"><AppIcon name="link" size={16} /></button><Link to={getGroupOpenPath(group)} onClick={() => onOpen(group)} aria-label={`Open ${group.name}`} className="inline-flex h-10 items-center gap-2 rounded-full bg-white px-4 text-sm font-black text-neutral-950 transition hover:bg-neutral-200"><AppIcon name="explore" size={16} />Open</Link></div></div><GroupContentOverview group={group} summary={summary} loading={summaryLoading} onOpenItem={onOpenItem} onOpenList={onOpenList} /></article>
}
