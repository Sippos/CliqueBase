...
function GroupCard({ group, summary, summaryLoading, onCopy, onOpen, onOpenItem, onOpenList }) {
  ...
  return <article className="rounded-[1.35rem] border border-white/10 bg-white/[0.028] p-3 text-white shadow-xl shadow-black/10 transition hover:border-white/20 hover:bg-white/[0.04] sm:rounded-[1.65rem] sm:p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.22em] text-neutral-500 sm:text-[10px] sm:tracking-[0.24em]">{group.isPublic ? 'Public clique' : 'Private clique'}</p><h2 className="mt-1 truncate text-xl font-black leading-tight sm:text-2xl">{group.name}</h2><div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-neutral-400 lg:hidden"><MemberAvatars members={group.members || []} /><span className="rounded-full border border-white/10 px-2.5 py-1 text-[11px] font-bold">{memberCount} {memberCount === 1 ? 'member' : 'members'}</span><button type="button" onClick={() => onCopy(group)} aria-label={`Copy invite for ${group.name}`} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-white transition hover:bg-white hover:text-neutral-950"><AppIcon name="link" size={15} /></button><Link to={getGroupOpenPath(group)} onClick={() => onOpen(group)} aria-label={`Open ${group.name}`} className="inline-flex h-9 items-center gap-1.5 rounded-full bg-white px-3 text-xs font-black text-neutral-950 transition hover:bg-neutral-200"><AppIcon name="explore" size={15} />Open</Link></div><div className="mt-3 hidden flex-wrap items-end gap-3 text-xs text-neutral-400 lg:flex"><MemberAvatars members={group.members || []} /><span className="mb-1 rounded-full border border-white/10 px-2.5 py-1 font-bold">{memberCount} {memberCount === 1 ? 'member' : 'members'}</span></div></div><div className="hidden shrink-0 flex-wrap gap-2 lg:flex"><button type="button" onClick={() => onCopy(group)} aria-label={`Copy invite for ${group.name}`} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-white transition hover:bg-white hover:text-neutral-950"><AppIcon name="link" size={16} /></button><Link to={getGroupOpenPath(group)} onClick={() => onOpen(group)} aria-label={`Open ${group.name}`} className="inline-flex h-10 items-center gap-2 rounded-full bg-white px-4 text-sm font-black text-neutral-950 transition hover:bg-neutral-200"><AppIcon name="explore" size={16} />Open</Link></div></div><GroupContentOverview group={group} summary={summary} loading={summaryLoading} onOpenItem={onOpenItem} onOpenList={onOpenList} /></article>
}
