...
function SmallIconButton({ icon, label, onClick, disabled = false, className = '' }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur transition hover:bg-white hover:text-neutral-950 disabled:pointer-events-none disabled:opacity-50 ${className}`}
    >
      <AppIcon name={icon} size={15} strokeWidth={2.4} />
    </button>
  )
}
...
function OverviewMetric({ icon, label, value, detail, onClick }) {
  const body = (
    <>
      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.05] text-neutral-200">
        <AppIcon name={icon} size={17} strokeWidth={2.3} />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-[10px] font-black uppercase tracking-[0.22em] text-neutral-500">{label}</span>
        <span className="mt-1 block text-xl font-black leading-none text-white">{value}</span>
        {detail ? <span className="mt-1 block truncate text-xs text-neutral-500">{detail}</span> : null}
      </span>
    </>
  )
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-left transition hover:-translate-y-0.5 hover:border-white/25 hover:bg-white/[0.06]">
        {body}
      </button>
    )
  }

  return <div className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3">{body}</div>
}

function CategoryOverviewRow({ category, loading, onOpen }) {
  const topTitle = category.top?.title || `No ${category.title.toLowerCase()} yet`
  const picks = category.items.reduce((sum, item) => sum + Number(item.picks || 0), 0)

  return (
    <button type="button" onClick={() => onOpen?.(category)} className="group flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-neutral-950/70 p-3 text-left transition hover:-translate-y-0.5 hover:border-white/25 hover:bg-white/[0.04]">
      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-neutral-950 shadow-lg shadow-black/20">
        <AppIcon name={category.icon} size={19} strokeWidth={2.5} />
      </span>
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-2 text-sm font-black text-white">
          {category.title}
          <span className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.16em] text-neutral-400">{loading ? '…' : plural(category.count, 'item')}</span>
        </span>
        <span className="mt-1 block truncate text-sm text-neutral-400">{loading ? 'Loading section…' : topTitle}</span>
      </span>
      <span className="hidden shrink-0 text-right sm:block">
        <span className="block text-sm font-black text-white">{loading ? '…' : picks}</span>
        <span className="text-[10px] uppercase tracking-[0.18em] text-neutral-500">picks</span>
      </span>
      <AppIcon name="chevronRight" size={16} className="shrink-0 text-neutral-500 transition group-hover:text-white" />
    </button>
  )
}
  ...
  return (
    <section className="mt-5 rounded-[1.6rem] border border-white/10 bg-neutral-950/70 p-3 shadow-2xl shadow-black/20">
      <div className="flex flex-wrap items-end justify-between gap-3 px-1">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-neutral-500">Overview</p>
          <h2 className="mt-1 text-xl font-black text-white">Your library at a glance</h2>
        </div>
        <p className="max-w-xs text-sm leading-6 text-neutral-500">Small counters first, section details underneath.</p>
      </div>

      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        <OverviewMetric icon="dashboard" label="Total" value={loading ? '…' : items.length} detail="all saved media" />
        {categories.map((category) => (
          <OverviewMetric key={category.title} icon={category.icon} label={category.title} value={loading ? '…' : category.count} detail={category.top?.title || 'empty section'} onClick={() => onOpen?.(category)} />
        ))}
        <OverviewMetric icon="info" label="Rated" value={loading ? '…' : ratedCount} detail="with rating data" />
        <OverviewMetric icon="users" label="Picks" value={loading ? '…' : totalPicks} detail="saved votes" />
      </div>

      <div className="mt-3 grid gap-2 lg:grid-cols-3">
        {categories.map((category) => <CategoryOverviewRow key={category.title} category={category} loading={loading} onOpen={onOpen} />)}
      </div>
    </section>
  )
}
...
function LibraryShowcase({ items, loading, onShare, onInfo }) {
  ...
  if (loading) {
    return (
      <div className="relative flex min-h-[260px] items-end overflow-hidden bg-neutral-950 p-5 xl:min-h-full">
        ...
        <div className="relative"><p className="text-xs uppercase tracking-[0.3em] text-neutral-500">Library feel</p><h2 className="mt-2 text-2xl font-black text-white">Loading your library…</h2></div>
      </div>
    )
  }
  ...
  if (!items.length) {
    return (
      <div className="relative flex min-h-[260px] items-end overflow-hidden bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.16),transparent_32%),linear-gradient(135deg,rgba(255,255,255,0.08),rgba(0,0,0,0.4))] p-5 xl:min-h-full">
        <div><p className="text-xs uppercase tracking-[0.3em] text-neutral-400">Empty library</p><h2 className="mt-2 text-2xl font-black text-white">Add the first item</h2><p className="mt-2 max-w-sm text-sm leading-6 text-neutral-300">Watched movies, finished series, and played games will appear here once you save them.</p></div>
      </div>
    )
  }
  ...
  return (
    <div className="relative min-h-[260px] overflow-hidden bg-neutral-950 xl:min-h-full">
      ...
      <div className="absolute bottom-5 right-5 z-10 flex gap-2">
        ...
        <SmallIconButton icon="share" label={`Share ${item.title}`} onClick={() => onShare?.(item)} className="bg-white text-neutral-950 hover:bg-neutral-200" />
      </div>
    </div>
  )
}

function CategorySpotlightCard({ category, loading, isClique, saving, onOpenPile, onInfo, onShare, onCopy }) {
  const top = category.top
  const image = top?.poster || top?.backdrop
  const summary = top ? itemText(top) : `Add ${category.title.toLowerCase()} to make this section feel alive.`

  return (
    <article className="overflow-hidden rounded-[1.6rem] border border-white/10 bg-neutral-950/80 shadow-2xl shadow-black/20">
      <button type="button" onClick={() => top ? onInfo?.(top) : onOpenPile?.(category)} className="group relative block h-48 w-full overflow-hidden text-left">
        ...
        <div className="absolute left-4 top-4 flex items-center gap-2 rounded-full border border-white/15 bg-black/60 px-3 py-1.5 text-xs font-black uppercase tracking-[0.18em] text-white backdrop-blur">
          <AppIcon name={category.icon} size={14} strokeWidth={2.4} />
          {category.title}
        </div>
        <div className="absolute bottom-4 left-4 right-4">
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-neutral-300">{loading ? 'Loading…' : plural(category.count, 'item')}</p>
          <h3 className="mt-1 line-clamp-2 text-2xl font-black leading-tight text-white">{loading ? 'Loading section…' : top?.title || `No ${category.title.toLowerCase()} yet`}</h3>
        </div>
      </button>
      <div className="space-y-4 p-4">
        <p className="line-clamp-3 text-sm leading-6 text-neutral-400">{summary}</p>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <button type="button" onClick={() => onOpenPile?.(category)} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-2 text-xs font-black uppercase tracking-[0.14em] text-neutral-300 transition hover:bg-white hover:text-neutral-950">
            <AppIcon name="list" size={14} strokeWidth={2.4} />
            Open list
          </button>
          <div className="flex gap-2">
            {top ? <SmallIconButton icon="info" label={`Show details for ${top.title}`} onClick={() => onInfo?.(top)} /> : null}
            {top ? <SmallIconButton icon="share" label={`Share ${top.title}`} onClick={() => onShare?.(top)} /> : null}
            {isClique && top ? <SmallIconButton icon="copy" label={`Copy ${top.title} to My Library`} onClick={() => onCopy?.(top)} disabled={saving} /> : null}
          </div>
        </div>
      </div>
    </article>
  ...
  return (
    <section className="mt-5 rounded-[2rem] border border-white/10 bg-white/[0.03] p-5" id="library-inline-list">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-white/10 pb-5">
        <div>
          <p className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-neutral-500"><AppIcon name={category.icon} size={14} />{isClique ? 'Clique list' : 'Library list'}</p>
          <h2 className="mt-1 text-3xl font-black text-white">{category.title}</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-400">{loading ? 'Loading…' : plural(items.length, 'item')} in this section.</p>
        </div>
              ...
              <article key={`${item.type}-${item.id}`} className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-neutral-950/80">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button type="button" onClick={() => onInfo?.(item)} className="rounded-2xl border border-white/10 px-3 py-2 text-sm font-black text-white transition hover:bg-white hover:text-neutral-950">Info</button>
                    ...
                    {isClique ? <button type="button" disabled={copyBusy} onClick={() => onCopy?.(item)} className="rounded-2xl border border-white/10 px-3 py-2 text-sm font-black text-white transition hover:bg-white hover:text-neutral-950 disabled:opacity-60">{copyBusy ? 'Copying…' : 'Copy'}</button> : null}
                    {isClique ? <button type="button" disabled={voteBusy} onClick={() => onVote?.(item, 'like')} className="rounded-2xl bg-white px-3 py-2 text-sm font-black text-neutral-950 transition hover:bg-neutral-200 disabled:opacity-60">{voteBusy ? 'Saving…' : 'Watch'}</button> : null}
                    {isClique ? <button type="button" disabled={voteBusy} onClick={() => onVote?.(item, 'dislike')} className="rounded-2xl border border-white/10 px-3 py-2 text-sm font-black text-white transition hover:bg-white hover:text-neutral-950 disabled:opacity-60">Pass</button> : null}
                  </div>
                </div>
              </article>
            )
    ...
    <PageShell active="library">
      <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.03] shadow-2xl shadow-black/20">
        <div className="grid gap-0 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="p-4 sm:p-6">
            <p className="text-xs font-black uppercase tracking-[0.3em] text-neutral-500">{isClique ? 'Clique' : 'Personal library'}</p>
            <h1 className="mt-2 max-w-3xl text-3xl font-black tracking-tight text-white sm:text-5xl">{context.name}</h1>
      ...
      <section className="mt-5 rounded-[2rem] border border-white/10 bg-white/[0.03] p-5">
        <div>
          ...
          <h2 className="mt-1 text-3xl font-black text-white">Top items by section</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-400">Quickly open the full Movies, Series, or Games list without the oversized overview buttons.</p>
        </div>
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          {categories.map((category) => {
            const item = category.top
            return <CategorySpotlightCard key={category.title} category={category} loading={loading} isClique={isClique} saving={copyingKey === itemActionKey(item, 'copy-')} onOpenPile={openPile} onInfo={setInfoItem} onShare={openShare} onCopy={copyToLibrary} />
          })}
        </div>
      </section>
