  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="max-h-[88vh] w-full max-w-5xl overflow-hidden rounded-[2rem] border border-white/10 bg-neutral-950 text-white shadow-2xl shadow-black/50">
        <div className="flex flex-col gap-4 border-b border-white/10 p-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-neutral-500"><AppIcon name={category.icon} size={14} />List overview</p><h2 className="mt-1 text-3xl font-black">{category.title} in this clique</h2><p className="mt-2 text-sm text-neutral-400">{items.length} card{items.length === 1 ? '' : 's'} · click any card for details.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => { onClose(); onOpenPile(category) }} className="rounded-2xl bg-white px-4 py-2 text-sm font-black text-neutral-950 hover:bg-neutral-200">Swipe pile</button><Link to={scopedHref(category, groupId)} onClick={onClose} className="rounded-2xl border border-white/10 px-4 py-2 text-sm font-black text-neutral-300 hover:bg-white hover:text-neutral-950">Open add/search</Link><button type="button" onClick={onClose} className="rounded-2xl border border-white/10 px-4 py-2 text-sm font-black text-neutral-300 hover:bg-white hover:text-neutral-950">Close</button></div></div>
        <div className="max-h-[62vh] overflow-y-auto p-5">{items.length ? <div className="grid gap-3 md:grid-cols-2">{items.map((item, index) => { const image = artFor(item); const saving = copyingKey === actionKey(item, 'copy-'); return <article key={`${item.type}-${item.id}`} className="group relative min-h-[17rem] overflow-hidden rounded-[1.5rem] border border-white/10 bg-neutral-950 transition hover:border-white/25">{image ? <img src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-80 transition duration-500 group-hover:scale-105" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.14),transparent_32%),linear-gradient(135deg,rgba(255,255,255,0.08),rgba(0,0,0,0.45))]" />}<div className="absolute inset-0 bg-gradient-to-t from-black via-black/45 to-black/5" /><button type="button" onClick={() => onInfo(item)} className="absolute inset-0 z-10 text-left" /><div className="pointer-events-none relative z-20 flex min-h-[17rem] flex-col justify-between p-4"><div className="flex items-start justify-between gap-3"><span className="rounded-full bg-white px-3 py-1.5 text-[10px] font-black text-neutral-950">#{index + 1}</span><div className="pointer-events-auto flex gap-2"><button type="button" onClick={(event) => { event.stopPropagation(); onInfo(item) }} className="h-9 w-9 rounded-full border border-white/15 bg-black/55 text-white hover:bg-white hover:text-neutral-950"><AppIcon name="info" size={15} /></button><button type="button" onClick={(event) => { event.stopPropagation(); onShare(item) }} className="h-9 w-9 rounded-full border border-white/15 bg-black/55 text-white hover:bg-white hover:text-neutral-950"><AppIcon name="share" size={15} /></button><button type="button" onClick={(event) => { event.stopPropagation(); onCopy(item) }} disabled={saving} className="h-9 w-9 rounded-full bg-white text-neutral-950 hover:bg-neutral-200 disabled:opacity-60"><AppIcon name="dashboard" size={15} /></button></div></div><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-neutral-300">{item.type}</p><h3 className="mt-1 line-clamp-2 text-2xl font-black leading-tight">{item.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-5 text-neutral-300">{detailsText(item)}</p><div className="mt-3 flex flex-wrap gap-2"><Pill>Score {item.score || 0}</Pill><Pill>{item.picks || 0} picks</Pill>{item.rating ? <Pill>Rating {Number(item.rating).toFixed(1)}</Pill> : null}</div></div></div></article> })}</div> : <p className="rounded-3xl border border-dashed border-white/10 p-6 text-sm leading-6 text-neutral-400">No {category.title.toLowerCase()} in this clique yet.</p>}</div>
      </div>
    </div>
  )
}

function SwipePilePanel({ category, groupId, swiped, copyingKey, onSwipe, onClose, onShowAll, onInfo, onShare, onCopy }) {
  if (!category) return null
  const items = category.items || []
  ...
  return (
    <section className="mt-5 rounded-[2rem] border border-white/10 bg-white/[0.03] p-5" id="clique-category-panel">
      <div className="flex flex-col gap-4 border-b border-white/10 pb-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-neutral-500"><AppIcon name={category.icon} size={14} />Swipe pile</p><h2 className="mt-1 text-3xl font-black text-white">{category.title} pile</h2><p className="mt-2 text-sm text-neutral-400">Swipe here, or open all items as a popup card list.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={onShowAll} className="rounded-2xl bg-white px-4 py-2 text-sm font-black text-neutral-950 hover:bg-neutral-200">Show all cards</button><Link to={scopedHref(category, groupId)} className="rounded-2xl border border-white/10 px-4 py-2 text-sm font-black text-neutral-300 hover:bg-white hover:text-neutral-950">Open add/search</Link><button type="button" onClick={onClose} className="rounded-2xl border border-white/10 px-4 py-2 text-sm font-black text-neutral-300 hover:bg-white hover:text-neutral-950">Close pile</button></div></div>
      <div className="mt-6"><SwipeDeck items={deckItems} onSwipe={(vote, item) => onSwipe(category, item, vote)} itemLabel={category.title.toLowerCase()} emptyLabel={`No ${category.title.toLowerCase()} left in this pile`} likeLabel="Keep" dislikeLabel="Pass" infoType={category.singular.toLowerCase()} /></div>
      <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.slice(0, 9).map((item) => { const image = artFor(item); const saving = copyingKey === actionKey(item, 'copy-'); return <article key={`${item.type}-${item.id}`} className="group relative min-h-[15rem] overflow-hidden rounded-[1.5rem] border border-white/10 bg-neutral-950 transition hover:border-white/25">{image ? <img src={image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-75 transition duration-500 group-hover:scale-105" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.14),transparent_32%),linear-gradient(135deg,rgba(255,255,255,0.08),rgba(0,0,0,0.45))]" />}<div className="absolute inset-0 bg-gradient-to-t from-black via-black/45 to-black/5" /><button type="button" onClick={() => onInfo(item)} className="absolute inset-0 z-10 text-left" /><div className="pointer-events-none relative z-20 flex min-h-[15rem] flex-col justify-between p-4"><div className="pointer-events-auto flex justify-end gap-2"><button type="button" onClick={(event) => { event.stopPropagation(); onShare(item) }} className="h-9 w-9 rounded-full border border-white/15 bg-black/55 text-white hover:bg-white hover:text-neutral-950"><AppIcon name="share" size={15} /></button><button type="button" onClick={(event) => { event.stopPropagation(); onCopy(item) }} disabled={saving} className="h-9 w-9 rounded-full bg-white text-neutral-950 hover:bg-neutral-200 disabled:opacity-60"><AppIcon name="dashboard" size={15} /></button></div><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-neutral-300">{item.type}</p><h3 className="mt-1 line-clamp-2 text-xl font-black leading-tight text-white">{item.title}</h3></div></div></article> })}</div>
    </section>
  )
}
...
export default function CliqueDetail() {
  ...
  function openPile(category) { setPileCategoryKey(category.key); setPileSwipes({}); window.setTimeout(() => document.getElementById('clique-category-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50) }
  ...
  return (
    <PageShell active="cliques">
      ...
      <SwipePilePanel category={pileCategory} groupId={groupId} swiped={pileSwipes} copyingKey={copyingKey} onSwipe={handlePileSwipe} onClose={() => setPileCategoryKey('')} onShowAll={() => pileCategory && setListCategoryKey(pileCategory.key)} onInfo={setSelectedItem} onShare={setSharingItem} onCopy={copyToLibrary} />
      ...
      <CategoryListModal category={listCategory} groupId={groupId} copyingKey={copyingKey} onClose={() => setListCategoryKey('')} onInfo={setSelectedItem} onOpenPile={openPile} onShare={setSharingItem} onCopy={copyToLibrary} />
