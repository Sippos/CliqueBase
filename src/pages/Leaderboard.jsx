...
function ExploreBoard() {
  ...
  function toggleTile(item, prefix = '') { const key = itemKey(item, prefix); setFlippedKey((current) => current === key ? null : key) }
  async function copyToLibrary(item) { setSavingItem(true); try { const payload = copyPayload(item); if (item.category === 'Movies') await saveMovie(payload, payload.nominated_by); else if (item.category === 'Series') await saveSeries(payload, payload.nominated_by); else if (item.category === 'Games') await saveGame(payload, payload.nominated_by); else throw new Error('This content type cannot be copied yet.'); setMessage(`${item.title} copied to your library.`); setSelectedItem(null); setFlippedKey(null); setTimeout(() => setMessage(null), 2500) } catch (error) { setMessage(error.message || 'Could not copy this pick.'); setTimeout(() => setMessage(null), 3500) } finally { setSavingItem(false) } }
