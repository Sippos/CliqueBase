...
function itemShareUrl(item) {
  ...
  if (item?.groupId) return `${window.location.origin}${getGroupOpenPath({ id: item.groupId })}`
  return `${window.location.origin}/explore`
}
function itemShareText(item) { return `${item?.title || 'CliqueBase pick'} · ${item?.groupName || 'Public Clique'}` }
function collectContributorStats(topContent = [], groups = []) {
  const sourceItems = [
    ...topContent,
    ...groups.flatMap((group) => (group.publicItems || group.topItems || []).map((item) => ({ ...item, groupId: item.groupId || group.id, groupName: item.groupName || group.name }))),
  ]
  const seen = new Set()
  const byName = new Map()
  sourceItems.forEach((item) => {
    const name = getNominator(item)
    if (!name) return
    const signature = `${name}-${item.category || ''}-${item.id || item.title || ''}-${item.groupId || item.groupName || ''}`
    if (seen.has(signature)) return
    seen.add(signature)
    const current = byName.get(name) || { name, count: 0, score: 0, ratings: [], items: [], topItem: null }
    ...
    if (!current.topItem || score > Number(current.topItem.score || 0)) current.topItem = item
    byName.set(name, current)
  })
  return Array.from(byName.values()).map((entry) => ({
    ...entry,
    items: sortByRank(entry.items).slice(0, 4),
    averageRating: entry.ratings.length ? entry.ratings.reduce((sum, value) => sum + value, 0) / entry.ratings.length : 0,
    highestRating: entry.ratings.length ? Math.max(...entry.ratings) : 0,
  })).sort((a, b) => b.count - a.count || b.averageRating - a.averageRating || b.score - a.score).slice(0, 12)
}
...
function FeaturedPickPile({ category, items, onInfo, onOpenLadder, onCopy, onShare, saving, showHeading = true, enableItemSwipe = true }) {
  ...
  return (
    <section className="sm:contents">
      ...
      <div className="relative pt-3 pr-1.5 sm:pt-8 sm:pr-5" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        ...
        <div className={`relative z-20 -mt-2 rounded-[1.05rem] border border-white/10 bg-black/52 p-1 shadow-xl shadow-black/25 backdrop-blur transition sm:-mt-3 sm:rounded-[1.2rem] sm:p-2.5 ${flipped ? 'hidden sm:block' : ''}`}>
          <div className="flex items-center justify-between gap-1.5">
            <button type="button" onClick={() => onInfo(activeItem)} className="min-w-0 flex-1 text-left">
              ...
              <p className="truncate text-[11px] font-black text-white sm:mt-0.5 sm:text-[11px] sm:font-normal sm:text-neutral-500"><span className="sm:hidden">#{displayRank} in {activeItem.groupName || 'Public clique'}{getNominator(activeItem) ? ` · ${getNominator(activeItem)}` : ''}</span><span className="hidden sm:inline">#{displayRank} of {items.length} · {activeItem.groupName || 'Public clique'}{getNominator(activeItem) ? ` · ${getNominator(activeItem)}` : ''}</span></p>
            </button>
            <button type="button" onClick={() => onOpenLadder(category)} aria-label={`Open ${meta.plural} ladder`} title="Open ladder" className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white text-neutral-950 transition hover:bg-neutral-200 sm:h-8 sm:w-8"><AppIcon name="list" size={13} strokeWidth={2.4} /></button>
          </div>
          <div className="mt-0.5 flex items-center justify-between gap-1.5 sm:mt-2 sm:gap-2">
            <div className="flex min-w-0 flex-wrap gap-1"><span className="rounded-full border border-white/10 bg-white/[0.04] px-1.5 py-0.5 text-[10px] font-bold text-neutral-200 sm:px-2 sm:text-[11px]">Score {activeItem.score || 0}</span><span className="rounded-full border border-white/10 bg-white/[0.04] px-1.5 py-0.5 text-[10px] font-bold text-neutral-200 sm:px-2 sm:text-[11px]">{activeItem.picks || 0} picks</span>{activeItem.rating ? <span className="rounded-full border border-white/10 bg-white/[0.04] px-1.5 py-0.5 text-[10px] font-bold text-neutral-200 sm:px-2 sm:text-[11px]">Rating {Number(activeItem.rating).toFixed(1)}</span> : null}</div>
...
function ShareSheet({ item, onClose, onFlash }) {
  ...
  async function copyLink() {
    try {
      ...
      onFlash('Share link copied.')
      onClose()
    } catch {
      onFlash('Could not copy the share link.', 3000)
    }
  }
  async function shareInApp() {
    try {
      if (navigator.share) await navigator.share({ title: item.title, text, url })
      else await navigator.clipboard?.writeText(`${text} ${url}`)
      onFlash(navigator.share ? 'Share sheet opened.' : 'CliqueBase link copied.')
      onClose()
    } catch (error) {
      if (error?.name !== 'AbortError') onFlash('Could not open sharing.', 3000)
    }
  }
  function whatsapp() {
  ...
  return <div className="fixed inset-0 z-[70] flex items-end bg-black/70 p-3 backdrop-blur-sm sm:items-center sm:justify-center" onClick={onClose}><article className="w-full rounded-[1.75rem] border border-white/10 bg-neutral-950 p-4 shadow-2xl shadow-black/60 sm:max-w-md" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.24em] text-neutral-500">Share pick</p><h2 className="mt-1 truncate text-xl font-black text-white">{item.title}</h2><p className="mt-1 truncate text-xs text-neutral-400">{item.groupName || 'Public Clique'}</p></div><button type="button" onClick={onClose} className="text-2xl text-neutral-500 transition hover:text-white">×</button></div><div className="mt-4 grid grid-cols-3 gap-2"><button type="button" onClick={shareInApp} className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.05] px-2 py-3 text-[11px] font-black text-white transition hover:bg-white hover:text-neutral-950"><AppIcon name="users" size={18} />CliqueBase</button><button type="button" onClick={copyLink} className="flex flex-col items-center justify-center gap-2 rounded-2xl bg-white px-2 py-3 text-[11px] font-black text-neutral-950 transition hover:bg-neutral-200"><AppIcon name="link" size={18} />Copy link</button><button type="button" onClick={whatsapp} className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.05] px-2 py-3 text-[11px] font-black text-white transition hover:bg-white hover:text-neutral-950"><AppIcon name="share" size={18} />WhatsApp</button></div><p className="mt-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs leading-5 text-neutral-400">Share with friends on CliqueBase, copy the public link, or send the pick through WhatsApp.</p></article></div>
}
...
function DetailModal({ item, saving, onCopy, onShare, onClose }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-3 backdrop-blur-sm sm:items-center sm:p-4"><article className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[1.75rem] border border-white/10 bg-neutral-950/95 shadow-2xl shadow-black/50 md:grid md:grid-cols-[0.72fr_1fr] md:overflow-hidden md:rounded-[2rem]"><div className="relative h-56 bg-neutral-900 sm:h-72 md:h-auto md:min-h-72">{displayItem.poster ? <img src={displayItem.poster} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-white"><AppIcon name={meta.icon} size={72} strokeWidth={1.5} /></div>}<div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/10 to-transparent md:hidden" /><div className="absolute left-4 top-4"><CategoryBadge category={displayItem.category} /></div></div><div className="p-4 sm:p-6 md:max-h-[90vh] md:overflow-y-auto"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><h2 className="text-2xl font-black leading-tight text-white sm:text-3xl">{displayItem.title}</h2><SourceLine item={displayItem} /></div><button type="button" onClick={onClose} className="text-2xl text-neutral-400 transition hover:text-white">×</button></div><dl className="mt-4 flex flex-wrap gap-2"><DetailRow label="Released" value={releaseValue} /><DetailRow label="Runtime" value={displayItem.runtime ? `${displayItem.runtime} min` : null} /><DetailRow label="Director / Regie" value={creator} /><DetailRow label="Seasons" value={displayItem.seasons} /><DetailRow label="Episodes" value={displayItem.episodes} /><DetailRow label="Platforms" value={displayItem.platforms || displayItem.platform} /></dl><GenreChips genres={displayItem.genres} />{detailsLoading ? <p className="mt-4 text-sm text-neutral-500">Loading API details…</p> : null}{detailsError && !hasMetadata(displayItem) ? <p className="mt-4 rounded-2xl border border-yellow-400/20 bg-yellow-400/10 p-3 text-xs leading-5 text-yellow-100">{detailsError}</p> : null}{displayItem.overview || displayItem.description ? <section className="mt-5 rounded-3xl border border-white/10 bg-white/[0.03] p-4"><h3 className="text-xs uppercase tracking-[0.22em] text-neutral-500">Overview</h3><p className="mt-2 text-sm leading-6 text-neutral-300">{displayItem.overview || displayItem.description}</p></section> : null}<div className="mt-5 flex flex-wrap gap-2"><MetricPill>Score {displayItem.score || 0}</MetricPill><MetricPill>{displayItem.picks || 0} picks</MetricPill>{displayItem.rating ? <MetricPill>Your rating {Number(displayItem.rating).toFixed(1)}</MetricPill> : null}{sourceRating !== null && sourceRating !== undefined ? <MetricPill>{displayItem.category === 'Games' ? 'RAWG' : 'TMDB'} {Number(sourceRating).toFixed(1)}</MetricPill> : null}{displayItem.completed ? <MetricPill active><WatchedLabel category={displayItem.category} /></MetricPill> : null}</div>{showEmptyMetadata ? <p className="mt-5 rounded-3xl border border-white/10 bg-white/[0.03] p-4 text-sm leading-6 text-neutral-400">No extra media metadata is available for this item yet.</p> : null}<div className="mt-5 grid grid-cols-2 gap-2 sm:flex sm:flex-row sm:items-center"><button type="button" onClick={() => onCopy(displayItem)} disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3 text-xs font-black text-neutral-950 transition hover:bg-neutral-200 disabled:opacity-60 sm:flex-1 sm:text-sm"><AppIcon name="copy" size={17} />{saving ? 'Copying...' : 'Copy'}</button><button type="button" onClick={() => onShare(displayItem)} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 px-4 py-3 text-xs font-black text-white transition hover:bg-white hover:text-neutral-950 sm:flex-1 sm:text-sm"><AppIcon name="share" size={17} />Share</button>{groupPath ? <Link to={groupPath} onClick={onClose} className="col-span-2 inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 px-4 py-3 text-xs font-black text-white transition hover:bg-white hover:text-neutral-950 sm:flex-1 sm:text-sm"><AppIcon name="explore" size={17} />Open clique</Link> : null}<button type="button" onClick={() => setCopyHintOpen((value) => !value)} aria-label="What does copy to my library do?" className="hidden h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-neutral-200 transition hover:bg-white hover:text-neutral-950 sm:inline-flex"><AppIcon name="info" size={20} strokeWidth={2.2} /></button></div>{copyHintOpen ? <p className="mt-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs leading-5 text-neutral-400">Copies this public pick into your personal library so you can rate it, mark it watched/played, or add it to your own cliques later.</p> : null}</div></article></div>
}
...
function ExploreBoard() {
  ...
  const contributors = useMemo(() => collectContributorStats(topContent, groups), [topContent, groups])
