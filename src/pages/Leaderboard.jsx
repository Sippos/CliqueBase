...
function collectContributorStats(topContent = [], groups = []) {
  ...
  sourceItems.forEach((item) => {
    ...
    seen.add(signature)
    const current = byName.get(name) || { name, count: 0, score: 0, ratings: [], topItem: null }
  ...
  return (
    <section className="sm:contents">
      ...
      <div className="relative pt-3 pr-1.5 sm:pt-8 sm:pr-5" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        ...
        <div className={`relative z-20 -mt-2 rounded-[1.05rem] border border-white/10 bg-black/52 p-1.5 shadow-xl shadow-black/25 backdrop-blur transition sm:-mt-3 sm:rounded-[1.2rem] sm:p-2.5 ${flipped ? 'hidden sm:block' : ''}`}>
          ...
          <div className="mt-1 flex items-center justify-between gap-2 sm:mt-2">
            <div className="flex min-w-0 flex-wrap gap-1"><span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[10px] font-bold text-neutral-200 sm:text-[11px]">Score {activeItem.score || 0}</span><span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[10px] font-bold text-neutral-200 sm:text-[11px]">{activeItem.picks || 0} picks</span>{activeItem.rating ? <span className="rounded-full border border-white/10 bg-white/[0.04] px-2 py-0.5 text-[10px] font-bold text-neutral-200 sm:text-[11px]">Rating {Number(activeItem.rating).toFixed(1)}</span> : null}</div>
...
function TopContributors({ contributors }) {
  ...
  return <section className="mt-6 sm:mt-8"><div className="mb-3 flex items-end justify-between gap-3 px-1"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-neutral-500 sm:text-xs">Community</p><h2 className="mt-1 text-2xl font-black text-white sm:text-3xl">Top contributors</h2></div><span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-neutral-300">{contributors.length} profiles</span></div><div className="-mx-1 flex gap-3 overflow-x-auto px-1 pb-2 [scrollbar-width:none]">{contributors.map((person) => <article key={person.name} className="w-[10.75rem] shrink-0 rounded-[1.35rem] border border-white/10 bg-white/[0.035] p-3 shadow-xl shadow-black/20 sm:w-56"><div className="flex items-center gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-sm font-black text-neutral-950">{initials(person.name)}</div><div className="min-w-0"><h3 className="truncate text-sm font-black text-white">{person.name}</h3><p className="truncate text-[11px] text-neutral-500">{person.topItem?.title || 'Public picks'}</p></div></div><div className="mt-3 grid grid-cols-2 gap-1.5"><MetricPill compact>{person.count} picks</MetricPill><MetricPill compact>Score {person.score}</MetricPill>{person.averageRating ? <MetricPill compact>Avg {person.averageRating.toFixed(1)}</MetricPill> : null}{person.highestRating ? <MetricPill compact>Best {person.highestRating.toFixed(1)}</MetricPill> : null}</div></article>)}</div></section>
}
